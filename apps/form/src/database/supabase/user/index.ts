import { supabaseForm } from '@/database/supabase';
...
export async function getCurrentUser(): Promise<Sessions | null> {
  let data = null;
  // 获取用户信息
  const [error, sessions] = await to(supabaseForm.auth.getSession());
  ...
  if (
    ...
    // 更新session
    const [error, data] = await to(supabaseForm.auth.getUser());
    if (error) return null;
    if (!data) return null;
    // 更新token成功获取session
    const [errors, sessions] = await to(supabaseForm.auth.getSession());
...
export async function logout() {
  const { error } = await supabaseForm.auth.signOut();
  if (error) return false;
  return true;
}
...
export async function signup({
  ...
  const { data, error } = await supabaseForm.auth.signUp({
    email,
    password,
    options: {
      data: {
...
export async function login({ email, password }: { email: string; password: string }) {
  const { data, error } = await supabaseForm.auth.signInWithPassword({
    email: email,
    password: password,
  });
  if (error) throw new Error('服务器错误');
...
export async function updateCurrentUser({
  ...
  // 更新用户信息
  const { data, error } = await supabaseForm.auth.updateUser(userData);
  if (error) throw new Error('服务器错误');
  return data.user;
}
