...
const Chat = observer(() => {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      socket.on('joinChat', (data: ActiveUser) => {
        chatStore.setConnectCount(data.roomSize);
        toast({
          description: <UserButtons user={data} />,
          variant: 'default',
        });
      });
      ...
      socket.on(
        ...
        (data: {
          ...
          const oldData = queryClient.getQueryData(['chat', activeWorkSpace.id]) as {
            pageParams: number[];
            pages: {
              messages: {
                data: Message[];
          ...
          queryClient.setQueryData(['chat', activeWorkSpace.id], {
  ...
  return (
    <ChatContainer>
      <Content>
        <ChatHeaderContainer>
          <ChatMeta isConnected={chatStore.isConnected} workspace={activeWorkSpace} />
        </ChatHeaderContainer>
