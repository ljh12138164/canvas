...
describe('UserColleciton', () => {
  it('加载状态下渲染 Loading 组件', () => {
    render(<UserColleciton data={undefined} loading={true} />);

    expect(screen.getByTestId('loading-component')).toBeInTheDocument();
  });

  it('无数据时渲染暂无数据提示', () => {
    render(<UserColleciton data={[]} loading={false} />);

    expect(screen.getByText('暂无数据')).toBeInTheDocument();
  });

  it('渲染收藏列表', () => {
    const mockData: UserCollectionResponseType = [
      {
        show: {
          id: 'show-1',
          title: '测试标题1',
    ...
    render(<UserColleciton data={mockData} loading={false} />);
    ...
    // 检查时间格式
    const times = screen.getAllByText('收藏时间：2023年01月01日');
