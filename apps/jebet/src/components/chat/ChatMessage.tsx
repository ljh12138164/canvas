...
const ChatMessage = observer(({ message, workspace, userId }: ChatMessageProps) => {
  return (
    <MessageContainer>
      {message.pages
        .flatMap((item) => item.messages.data)
        .map((messageData) => {
          const sendUser = workspace.member.find((item) => item.userId === messageData.userId);
          if (sendUser) {
            return (
              <MessageDescpt isSelf={sendUser.userId === userId}>
                ...
                {messageData.type === MessageType.TEXT && <MessageItem message={messageData} />}
