...
import { Link, MoreHorizontalIcon, MoreVertical, PencilIcon, Trash } from 'lucide-react';
...
const ProjectOpacte = observer(({ task, type }: { task: TaskWithWorkspace; type: 'kanban' | 'list' }) => {
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent>
        <DropdownMenuItem asChild>
          <Button variant="ghost" className="w-full cursor-pointer" onClick={() => navigate(`${task.id}`)}>
            <Link />
            <span>细节</span>
          </Button>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <TaskFromCard workspaceId={workspaceId} projectId={projectId} defaultData={task} type="edit" userData={task.workspace.member} currentUserId={userData.id}>
            <Button
              variant="ghost"
              className="w-full cursor-pointer"
              onClick={(e) => {
