...
onBeforeMount(async () => {
  ...
  if (formData.value && Object.keys(formData.value).length) {
    formData?.value.forEach((item) => {
      const schema = getZodSchema(item, fieldConfig) as z.ZodObject<
        any,
        any,
        any,
        any
...
<template>
  ...
  >
    ...
    <div
      v-else
      class="flex flex-col items-center justify-center h-[calc(100dvh-120px)]"
    >
      <p class="text-center text-sm text-gray-500">暂无数据</p>
      <Button class="w-[100px] py-2 transition-all" @click="router.back()">
        返回
      </Button>
    </div>
  </ScrollArea>
