...
const onSubmit = handleSubmit((values) => {
  ...
  list2.value.forEach((item) => {
    const schemas = getZodSchema(item, fieldConfig) as z.ZodObject<
      any,
      any,
      any,
      any
...
<template>
  <section
    class="flex flex-col px-1 gap-2 w-full rounded h-[calc(100dvh-170px)] overflow-hidden"
  >
    <div
      ref="parent"
      ...
      <ScrollArea class="h-[calc(100dvh-120px)] w-full bg-gray-500/5">
        ...
        >
          <div
            v-for="item in list1"
            :key="item.id"
            class="cursor-move h-50px rounded p-3 border"
          >
            {{ item.name }}
          </div>
        </VueDraggable>
