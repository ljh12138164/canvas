...
import { createWorkspace, getDoc, getWorkspaceById, getWorkspaces } from '../../../server/note/workspace';

const workspace = new Hono()
  .use(checkToken(process.env.SUPABASE_NOTE_JWT!))
  // 获取工作区
  .get('/workspaces/:workspaceId', zValidator('param', z.object({ workspaceId: z.string() })), async (c) => {
    const { token, auth } = getSupabaseAuth(c);
    const workspaceId = c.req.valid('param').workspaceId;
    const [error, workspace] = await to(getWorkspaceById({ token, userId: auth.sub, workspaceId }));
